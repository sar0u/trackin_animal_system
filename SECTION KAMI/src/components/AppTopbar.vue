...
// Base de données statique des pages de ton site pour la recherche rapide
const siteModules = [
  ...
  { title: 'Mouvements & Stats', route: '/mouvements', icon: 'fas fa-chart-line', type: 'Analytique', colorClass: 'bg-gray' },
  { title: 'Gestion Fraude', route: '/gestion-fraude', icon: 'fas fa-shield-alt', type: 'Sécurité & Inspections', colorClass: 'bg-red' },
  { title: 'Journal d\'Audit', route: '/audit', icon: 'fas fa-list-alt', type: 'Intégrité Système', colorClass: 'bg-navy' }
];

// Filtrer les résultats en temps réel selon ce que l'utilisateur tape
...
// Cache le menu déroulant quand on clique ailleurs (avec un petit délai pour laisser le temps au clic de marcher)
const hideDropdown = () => {
  setTimeout(() => {
...
const navigateTo = (path) => {
  ...
  showDropdown.value = false; // On ferme le menu
  router.push(path); // On redirige vers la vraie page
};
...
.avatar-circle {
  ...
  font-weight: 800; font-size: 15px; border: 2px solid #22c55e;
}
</style>