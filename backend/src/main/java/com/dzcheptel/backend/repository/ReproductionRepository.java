package com.dzcheptel.backend.repository;

import com.dzcheptel.backend.entity.Reproduction;
import com.dzcheptel.backend.entity.ReproductionStatus;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;

import java.util.List;
import java.util.Optional;

public interface ReproductionRepository extends JpaRepository<Reproduction, Long> {

    @Query("SELECT DISTINCT r FROM Reproduction r "
            + "LEFT JOIN FETCH r.female f LEFT JOIN FETCH f.rfidTagEntity LEFT JOIN FETCH f.owner LEFT JOIN FETCH f.farm "
            + "LEFT JOIN FETCH r.male m LEFT JOIN FETCH m.rfidTagEntity LEFT JOIN FETCH m.owner LEFT JOIN FETCH m.farm "
            + "LEFT JOIN FETCH r.veterinarian")
    List<Reproduction> findAllFetched();

    @Query("SELECT DISTINCT r FROM Reproduction r "
            + "LEFT JOIN FETCH r.female f LEFT JOIN FETCH f.rfidTagEntity LEFT JOIN FETCH f.owner LEFT JOIN FETCH f.farm "
            + "LEFT JOIN FETCH r.male m LEFT JOIN FETCH m.rfidTagEntity LEFT JOIN FETCH m.owner LEFT JOIN FETCH m.farm "
            + "LEFT JOIN FETCH r.veterinarian "
            + "WHERE r.id = :id")
    Optional<Reproduction> findFetchedById(Long id);

    List<Reproduction> findByFemaleId(Long femaleId);

    List<Reproduction> findByStatus(ReproductionStatus status);
}
