...
class AnimalRepository {
  ...
  static String _toBackendStatus(String french) {
    switch (french) {
      case 'ACTIF':  return 'Active';
      case 'VENDU':  return 'Sold';
      case 'PERDU':  return 'Lost';
      case 'MORT':   return 'Dead';
      case 'ABATTU': return 'Slaughtered';
      default:       return french;
    }
  }

  Future<AnimalModel> updateAnimal(String rfidTag, Map<String, dynamic> data) async {
    try {
      final encodedRfid = Uri.encodeComponent(rfidTag);
      if (data.containsKey('status')) {
        data = Map.from(data)
          ..['status'] = _toBackendStatus(data['status'] as String);
      }
      final response = await _apiClient.dio.put(
        '${ApiConstants.updateAnimal}$encodedRfid',
        data: data,
      );
